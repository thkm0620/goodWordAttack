...
class knnClassifier:
    ...
    check=0
    model = neighbors.KNeighborsClassifier(n_neighbors=21,algorithm='auto')
    ...
    def predict(msg):
        if knnClassifier.check==0:

            # Create training data
            xTrain, xTest, yTrain, yTest = knnClassifier.store()

            knnClassifier.vectorizer = TfidfVectorizer(stop_words='english', max_df=75)
        
            yTrainMatrix = np.asarray(yTrain)
            xTrainMatrix = knnClassifier.vectorizer.fit_transform(xTrain)

            # Training kNN classifier
            knnClassifier.model.fit(xTrainMatrix, yTrainMatrix)
            fScore, precision, recall, matrix = knnClassifier.calcFScore(xTest, yTest, knnClassifier.model, knnClassifier.vectorizer)
            print("fScore, precision, recall :")
            print(fScore, precision, recall)
            knnClassifier.check=1
            
        return knnClassifier.predict2(msg,knnClassifier.vectorizer,knnClassifier.model)
...
'''
print(knnClassifier.predict("FreeMsg: Claim ur 250 SMS messages-Text OK to 84025 now!Use web2mobile 2 ur mates etc. Join Txt250.com for 1.50p/wk. T&C BOX139, LA32WU. 16 . Remove txtX or stop"))
print(knnClassifier.predict("FREE for 1st week! No1 Nokia tone 4 ur mob every week just txt NOKIA to 87077 Get txting and tell ur mates. zed POBox 36504 W45WQ norm150p/tone 16+"))
print(knnClassifier.predict("I have a tad issue here about the thorough refining column"))
'''
